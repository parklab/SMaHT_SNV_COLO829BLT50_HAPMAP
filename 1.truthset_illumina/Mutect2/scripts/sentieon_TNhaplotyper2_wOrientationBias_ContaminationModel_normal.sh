...
while getopts ":s:i:r:p:t:b:n:m:" opt; do
  case $opt in
...
# Add processed normal BAMs
for normal_bam in ${normal_bams[*]}; do
    replace_args=$(process_bam_header "$normal_bam" "$normal_name")
    input_files+=" $replace_args -i $normal_bam "
done
...
command+=" --algo TNhaplotyper2 --tumor_sample $sample_name \
    ...
    output.vcf.gz"
command+=" --algo OrientationBias --tumor_sample $sample_name \
    output.priors"
command+=" --algo ContaminationModel --tumor_sample $sample_name \
    --normal_sample $normal_name \
...
# ******************************************
ulimit -s 65536 # Argument list can potentially be too long otherwise (default is 8192)
eval $command || exit 1
